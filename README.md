...
2. [Model Screening and Baseline Selection](#2-model-screening-and-baseline-selection)
3. [Fine Modeling: Hyperparameter Optimization](#3-fine-modeling-hyperparameter-optimization)
4. [Backtesting: Strategy Evaluation and Walk-Forward Analysis](#4-backtesting-strategy-evaluation-and-walk-forward-analysis)
...
1. A fresh model is trained on the historical data available up to that point. This prevents look-ahead bias.
2. The newly trained model then generates predictions for the next, unseen test period.
3. The Backtest framework applies your chosen strategy (`ContinuousAllocationStrategy`) to this new, unseen data.
4. The results from each fold are aggregated to produce a final, robust performance report and a combined equity curve.
